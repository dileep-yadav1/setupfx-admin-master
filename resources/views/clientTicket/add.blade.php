    ...
    <div class="row">
        ...
        <div class="col-xl-8">
            <div class="card">
                <div class="card-body">
                    @if ($aRow)
                        <form method="POST" action="{{ route('client-ticket.update', $aRow->id) }}"
                            enctype="multipart/form-data">
                            @method('PUT')
                        @else
                            <form method="POST" action="{{ route('client-ticket.store') }}" enctype="multipart/form-data">
                    @endif

                    @csrf
                    <div class="mb-3">
                        <label for="formrow-firstname-input" class="form-label">Department</label>
                        {{ Form::select('department', ['' => 'Please Select'] + $aDepartment, $aRow ? $aRow->department : null, ['class' => 'form-control', 'required' => 'required']) }}
                    </div>
                    <div class="mb-3">
                        <label for="formrow-firstname-input" class="form-label">Subject</label>
                        <input type="text" id="subject" name="subject"
                            class="form-control{{ $errors->has('subject') ? ' is-invalid' : '' }}"
                            value="{{ $aRow ? $aRow->subject : old('subject') }}" required placeholder="Subject">
                    </div>
                    <div class="mb-3">
                        <label for="formrow-email-input" class="form-label">Reporter</label>
                        {{ Form::select('reporter', ['' => 'Please Select'] + $getSalesAgent, $aRow ? $aRow->reporter : null, ['class' => 'form-control', 'required' => 'required']) }}
                    </div>
                    <div class="mb-3">
                        <label for="formrow-password-input" class="form-label">Priority</label>
                        {{ Form::select('priority', ['' => 'Please Select'] + $aPriority, $aRow ? $aRow->priority : null, ['class' => 'form-control', 'required' => 'required']) }}
                    </div>
                    <div class="mb-3">
